namespace BF1.MemoryWebAPI.Features.Core;
...
public static class Memory
{
    ...
    public static bool Initialize()
    {
        ...
        {
            var ProcessName = "bf1";
            Console.WriteLine($"目标程序名称 {ProcessName}");
            ...
            if (pArray.Length > 0)
            {
                ...
                Console.WriteLine($"目标程序窗口句柄 {windowHandle}");
                processId = process.Id;
                Console.WriteLine($"目标程序进程ID {processId}");
                processHandle = WinAPI.OpenProcess(
                    ProcessAccessFlags.VirtualMemoryRead |
                    ProcessAccessFlags.VirtualMemoryWrite |
                    ProcessAccessFlags.VirtualMemoryOperation,
                    false, processId);
                Console.WriteLine($"目标程序进程句柄 {processHandle}");
                if (process.MainModule != null)
                {
                    processBaseAddress = process.MainModule.BaseAddress.ToInt64();
                    Console.WriteLine($"目标程序主模块基址 0x{processBaseAddress:x}");
                    return true;
                }
                else
                {
                    Console.WriteLine($"发生错误，目标程序主模块基址为空");
                    return false;
                }
            }
            else
            {
                Console.WriteLine($"发生错误，未发现目标进程");
                return false;
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"战地1内存模块初始化异常 " + ex.Message);
            return false;
        }
    }
